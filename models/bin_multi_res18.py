...
model1 = torchvision.models.resnet18().to(device) # true 옵션으로 사전 학습된 모델을 로드
model2  = torchvision.models.resnet18().to(device)

PATH1 = './weight5_bin_normal_new'
PATH2 = './weight5_res18_multi_new'
...
def test(model1, model2, params):
    ...
    with torch.no_grad():
        for X, Y in tqdm(test_dataloader):
            ...
            correct_bin_y.append(Y_bin)
            pred_bin_y.append(predicted)
            correct_all.append(Y)
            ...
            _, predicted = torch.max(outputs.data, 1)
            correct_multi_y.append(Y_multi)
            pred_multi_y.append(predicted)


    correct_all = torch.cat(correct_all, axis=0)

    a = np.array(correct_all.cpu().numpy())
    np.save('./res18_all_label_new', a)
    ...
    np.save('./res18_multi5_all_new', a)
